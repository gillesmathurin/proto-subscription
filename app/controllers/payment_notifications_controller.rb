class PaymentNotificationsController < ApplicationController
  ...
  def subscription_payment_notification?(params)
    params[:payment_status] == "Completed" && params[:receiver_email] == APP_CONFIG[:paypal_email] && params[:payment_gross] == plan_price(params[:invoice]) && params[:txn_type] == "subscr_payment" && params[:mc_currency] == "USD" && params[:mc_gross] == plan_price(params[:invoice])
  end
